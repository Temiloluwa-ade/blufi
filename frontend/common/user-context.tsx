...
type FclContextProps = {
  ...
  fetchCurrentUserInfo: KeyedMutator<bluFiInfo | null>;
  getInfo: (address: string) => Promise<bluFiInfo | null>;
  donateFlow: (
    message: string,
    amount: number,
    recurring: boolean,
...
export function FclProvider({ children }: { children: ReactChild }) {
  ...
  const { data: info, mutate: fetchCurrentUserInfo } =
    useSWR<bluFiInfo | null>(`info/${user?.addr}`, () =>
