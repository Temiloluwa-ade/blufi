...
export default function UserProfile({
  ...
  const [bluFiAmount, setbluFiAmount] = useState(0);
  ...
  async function onConfirmTx() {
    ...
    try {
      await donateFlow(message, bluFiAmount, recurring, address);
      resetValues();
      toast.success(`You successfully donated ${bluFiAmount}FLOW!`);
    } catch (e) {
      console.error(e);
      ...
      >
        ...
        {!isSelf && (
          <Shadow className="buy-flow-tea-form">
            <h5>Tip {info?.name} with flow</h5>
            <ChooseFlowAmount
              onChange={setbluFiAmount}
              value={bluFiAmount}
            />
            <RepeatPaymentSwitch
              style={{ marginTop: 50 }}
